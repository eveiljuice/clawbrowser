/**
 * Daemon client — sends HTTP requests to daemon or falls back to direct launch.
 */
...
/**
 * Send a command to the daemon. Returns null if no daemon running.
 */
...
/**
 * Connect to daemon's Chrome or launch a new one.
 * Only used as fallback when daemon is not running.
 */
export async function connectOrLaunch(opts: {
  proxy?: string;
  headless?: boolean;
} = {}): Promise<DaemonConnection> {
  // No daemon check here — caller should use daemonRequest first
